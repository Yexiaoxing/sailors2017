...
def read_csv(path):
    ...
    with open(path) as f:
        ...
        for row in reader:
            ...
            assert tweetId not in list(data.keys())
...
def classify_nb(tweet, prior_probs, token_probs):
    ...
    return max(list(log_posteriors.keys()), key=lambda c: log_posteriors[c])
...
def visualize_tweet(tweet, prior_probs, token_probs):
    ...
    for token in tokens:
        ...
        html += '<span style="%s" class="viz-token" data-food="%f" data-none="%f" data-medical="%f" data-energy="%f" data-water="%f">%s</span> ' \
                % (class2color_style(max_class), token_probs['Food'][token], token_probs['None'][token],
                   token_probs['Medical'][token],
